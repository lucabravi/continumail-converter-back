...
- **Thunderbird flag & tag fidelity:** from a **live Thunderbird profile**, the converter reads each folder's `.msf` index to recover **read/unread, replied, forwarded, starred** (→ a yellow **"Star"** category, not a To-Do follow-up flag), **junk**, and **tags** — and turns tags into Outlook **categories** using your real tag names. (Without a profile, these are recovered only from inline `X-Mozilla-Status` flags where present — see [Limitations](#-limitations).)
